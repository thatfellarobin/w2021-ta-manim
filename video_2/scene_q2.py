...
class P2(GraphScene):
    ...
    def construct(self):
        ...
        # Text
        y = MathTex('y(x)', '=', '\\frac{k}{x}', color=BLUE).scale(0.8).shift(2.75*RIGHT + 3*UP)
        ...
        rho = MathTex('\\rho(x)', '=', '\\frac{\\sqrt{\\left(1+y^{\\prime}(x)^2\\right)^3}}{y^{\\prime\\prime}(x)}').scale(0.8).next_to(y_prime, DOWN, aligned_edge=LEFT)
        y_double_prime = MathTex('y^{\\prime\\prime}(x)', '=', '\\frac{d}{dx}y^{\\prime}(x)', '=', '\\frac{2k}{x^3}').scale(0.8).next_to(rho, DOWN, aligned_edge=LEFT)
        ...
        a_solution = MathTex(
            ...
            '\\mathrm{mm/s^2}')
        a_solution.scale(0.8).next_to(y_double_prime, DOWN, aligned_edge=LEFT)
